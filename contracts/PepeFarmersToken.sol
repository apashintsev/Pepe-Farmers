...
contract PepeFarmersToken is Ownable, ERC20 {
    ...
    ) ERC20("Pepe Farmers", "Pepe Farmers") {
    ...
    function setExcludeFromFee(address user) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (pancakeswapV2Pair != address(0) && from == pancakeswapV2Pair) {
            uint256 fee = _isExcludedFromFees[to] ? 0 : marketingFee;
            if (fee > 0) {
                payable(marketingWallet).transfer(fee);
            }
        }
    }
}
